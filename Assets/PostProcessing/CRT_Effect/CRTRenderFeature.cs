...
//            if (currentCam.tag != settings.mainCameraTag) return;
...
//            crtMaterial.SetFloat("_Saturation", settings.saturation);
//            crtMaterial.SetFloat("_Contrast", settings.contrast);
//            crtMaterial.SetInt("_FlipUV", settings.flipMainCameraUV ? 1 : 0); // 传递翻转开关

//            // 3. 安全获取主相机渲染目标
//            RenderTargetIdentifier mainCamTarget = renderingData.cameraData.renderer.cameraColorTarget;

//            // 4. 执行CRT效果（仅用基础Blit，4参数，无兼容问题）
//            cmd.GetTemporaryRT(tempRT.id, rtDesc, FilterMode.Bilinear);
//            // 基础Blit重载：cmd, 源, 目标, 材质, pass索引（无矩阵）
//            Blit(cmd, mainCamTarget, tempRT.Identifier(), crtMaterial, 0);
//            Blit(cmd, tempRT.Identifier(), mainCamTarget);
...
//        {
//            Debug.LogError("[CRT] 请指定有效的CRTPostEffect.shader！");
//            return;
//        }
