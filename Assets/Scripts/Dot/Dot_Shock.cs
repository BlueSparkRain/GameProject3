public class Dot_Shock : DotBase{
    ...
    /// </summary>
    void DotShock(){
        UnityEngine.Debug.Log($"{self.Camp}触发{weaknessType},当前层数:{dot_count}");
        ...
        if (self.GetWeakAttack(weaknessType)){
            ...
            UnityEngine.Debug.Log($"{self.Camp}受{dot_type} Dot伤害,造成一次[(弱点)]伤害:[已损失生命值]{lostHp}*[Dot层数]{dot_count}*[弱点倍率]{weakMulti}={baseDamage}");
            self.battleDamageHandler.DoModelValue(E_BattleModelType.ShieldPoints, -1);
        }
        else{
            UnityEngine.Debug.Log($"{self.Camp}受{dot_type} Dot伤害,造成一次伤害:[已损失生命值]{lostHp}*[Dot层数]{dot_count}={baseDamage}");
        }
