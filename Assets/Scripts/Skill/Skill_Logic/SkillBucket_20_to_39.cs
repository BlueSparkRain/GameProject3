...
public class Skill_20 : SkillBase
{
    ...
    /// <summary>
    /// 【生命偷取+1】变为【生命偷取+2】（生命偷取+30%）/【生命偷取+3】（生命偷取+45%）/【生命偷取+4】（生命偷取+60%）
    /// </summary>
    ...
    void CreateBuff(int level)
    {
        ...
        if (buff != null)
        {
            Debug.Log("[Skill 20]:替换掉上一个的增益效果，刷新计时");
...
public class Skill_21 : SkillBase
{
    ...
    /// <summary>
    /// 【治疗加成+1】变为【治疗加成+2】（治疗加成+30%）/【治疗加成+3】（治疗加成+45%）/【治疗加成+4】（治疗加成+60%）
    /// </summary>
    /// <param name="target"></param>
    /// <param name="henceTime"></param>
    public override void SkillEffect_Enhence(IBattlable target, int henceTime){
    ...
    void CreateBuff(int level){
        BattleBuffHandler buffHandle = self.battleDamageHandler.BuffHandler;
        Battle_Controller battleControl = self.battleDamageHandler.BattleController;
        Debug.Log($"[Skill 21]{self.Camp}发动技能21-[治疗效果+N（{level}）-BUFF]");
        if (buff != null){
            Debug.Log("[Skill 21]:替换掉上一个的增益效果，刷新计时");
            battleControl.AdjustCharacterPropertyValue(E_CharacterPropertyType.Heal_Amplification, -adjustValue);
        }
        adjustValue = baseRiseRate * level;
        buff = new Buff_AdjustProperty(E_BuffType.生命偷取加N, E_BuffPositive.正面, buff_duration, battleControl, E_CharacterPropertyType.Heal_Amplification, adjustValue);
        //为目标注册BUFF        
        EventCenter.EventTrigger(E_EventType.Battle_RegisteBUFF, buffHandle, buff);
    }
}
...
public class Skill_22 : SkillBase
{
    ...
    /// <summary>
    ///找到BuffHandle，查找其中的BUFF名称，依次移除
    /// </summary>
    void DeletePropertyNagtiveBuff()
    {
        var buffHanlder = self.battleDamageHandler.BuffHandler;
        buffHanlder.UnRegistBuff(E_BuffType.物攻减N);
        buffHanlder.UnRegistBuff(E_BuffType.魔攻减N);
        buffHanlder.UnRegistBuff(E_BuffType.物防减N);
        buffHanlder.UnRegistBuff(E_BuffType.魔防减N);
    }
    public override void SkillEffect_Base(IBattlable target)
    {
