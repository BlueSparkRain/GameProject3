...
public class BattleBuffHandler : MonoBehaviour{
    ...
    public void UnRegistBuff(E_BuffType buffType) {
        ...
        Debug.Log($"{self.battleDamageHandler.name}未找到BUFF：{buffType}，移除失败");
    }
    ...
    public void UnRegistBuffsByAttr(E_BuffPositive attr) {
        ...
        foreach (var key in toRemove) {
            Debug.Log($"{self.battleDamageHandler.name}移除了BUFF：{key.Buff_Type}");
            key.OnBuffRemove();
            BuffDic.Remove(key);
        }
    }
    ...
    public E_SkillTargetType GetModifiedTargetType(E_SkillTargetType original, bool isMagic) {
        if (!isMagic && TryGetBuff(E_BuffType.无双_正面) != null) {
            Debug.Log($"{self.battleDamageHandler.name}无双发动：物理技能目标变为全体");
            return E_SkillTargetType.对全体;
        }
        if (isMagic && TryGetBuff(E_BuffType.魔力收束_正面) != null) {
            Debug.Log($"{self.battleDamageHandler.name}魔力收束发动：魔法技能目标变为单体");
            return E_SkillTargetType.对单体;
        }
        return original;
    }
    ...
    public float GetDamageRate() {
        ...
        else {Debug.Log(self.battleDamageHandler.name+"获得了伤害倍率BUFF,倍率:"+Buff.BoomerRate);
